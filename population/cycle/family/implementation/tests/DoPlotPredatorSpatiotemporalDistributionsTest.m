classdef DoPlotPredatorSpatiotemporalDistributionsTest < matlab.unittest.TestCase
  ...
  methods
    function fakeFcnToPassIn(testCase,currentDirName,load,...
      ...
      testCase.passedInGetSolutionPart = getSolutionPart;
  ...
  methods (Test)
    function testPassesParamsToPassedInFunction(testCase)
      ...
      testCase.verifyEqual(testCase.passedInGetPeriod,...
        @getPeriod,...
        'Передана неправильная функция получения периода решения');
      testCase.verifyEqual(testCase.passedInGetSolutionPart,...
        @getSolutionPart,...
        'Передана неправильная функция получения части решения');
