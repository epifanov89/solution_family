function predator_prey_1x2_nonuniform_resource_with_cosymmetry()
%PREDATOR_PREY_1x2_NONUNIFORM_RESOURCE_WITH_COSYMMETRY Модель взаимодействия двух популяций
...
[carrying_capacity_integral_array,...
...
global poincare_points poincare_times
...
ic_matrix(N+1:2*N,ic_index) = prey0;
...
plot_phase_trajectories(solutions);
  ...
  function analyze_solution_and_output_results(t_arr,w)
    ...
    if isperiodic
      ...
      w0 = w(end, :);
      multiplicators(@monodromy_matrix_one_system_for_each_column,...
                     @f, @g, @ode15s, [0 T], w0, nvar,...
                     odeset('OutputFcn',@odephas2,...
                            'OutputSel',[first_oscillating_var_index,...
                                         second_oscillating_var_index]))
    end
  ...
  function [carrying_capacity_integral_array,...
      inverse_carrying_capacity_integral_array] = compute_integrals()
...
%   end
  ...
  function [value,isterminal,direction] = events(t,y)
    value = y(fixed_var_index)-w0(fixed_var_index);
    isterminal = 1;
    direction = 1;
  end
