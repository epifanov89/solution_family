function predator_prey_const_resource()
%PREDATOR_PREY_CONST_RESOURCE Модель взаимодействия одной популяции
%жертвы и одной популяции хищника с ростом жертв, пропорциональным квадрату 
%плотности популяции, при однородной функции ресурса
...
search_activity = [0; 0; 0];
% Измените, чтобы поменять коэффициенты диффузии
diffusion_coeffs = [0.2; 0.3; 0.3];
...
if ~steady
  figure
  w = myode4(@f,tspan,w0,odeset('NonNegative',1:nvar,...
                                  'OutputFcn',@odephas2,...
  ...
  wout = zeros(1,N+1);
  wout(1,1:N) = w(end,N+1:2*N);
  wout(1,N+1) = w(end,N+1);
  ...
  %figure
  %hold on
  %h2 = plot(tspan,w(:,first_predator_center_point_var_index));
  ...
  figure
  calculate_multiplicators(wend,[0 T],@ode15s,...
                           odeset('NonNegative',1:nvar,'RelTol',1e-6),...
                           odeset('OutputFcn',@odephas2,...
                                  'OutputSel',[first_oscillating_var_index,second_oscillating_var_index]));
  
  figure
  [t,w] = ode15s(@f,[0 2*T],wend,...
  ...
  function inverse_carrying_capacity_integral_array = compute_integrals()
    ...
    for point_index = 1:N
      ...
      % отрезку
      inverse_carrying_capacity_integral_array(point_index) = 1;%integral(@(x) carrying_capacity(x).^(-1), xstart, xend) / h;
    end
  end
