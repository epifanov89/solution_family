function predator_prey_2x1_nonuniform_resource_with_cosymmetry()
%PREDATOR_PREY_2x1_NONUNIFORM_RESOURCE_WITH_COSYMMETRY Модель взаимодействия одной популяции
...
%   ic_matrix(2*N+point_index,ic_index)=u10((point_index-1)*h);
% end
% 
% ic_index = 2;
% for point_index = 1:N
...
% ic_index = 3;
% for point_index = 1:N
%   ic_matrix(point_index,ic_index)=u40((point_index-1)*h);
%   ic_matrix(N+point_index,ic_index)=u60((point_index-1)*h);
%   ic_matrix(2*N+point_index,ic_index)=u50((point_index-1)*h);
% end
% 
% ic_index = 4;
...
% predator0(1:N) = 0.6;
% 
% ic_matrix(1:N,ic_index) = prey0;
% ic_matrix(N+1:2*N,ic_index) = predator0;
% ic_matrix(2*N+1:3*N,ic_index) = predator0;
% 
% ic_index = 5;
% for point_index = 1:N
%   ic_matrix(point_index,ic_index)=u70((point_index-1)*h);
%   ic_matrix(N+point_index,ic_index)=u80((point_index-1)*h);
%   ic_matrix(2*N+point_index,ic_index)=u60((point_index-1)*h);
% end
...
first_oscillating_var_index = (first_oscillating_species-1)*N+fix((N+1)/4);
second_oscillating_var_index = (second_oscillating_species-1)*N+fix((N+1)/4);
...
for point_index = 1:N
  ic_matrix(point_index,ic_index)=u40((point_index-1)*h);
  ic_matrix(N+point_index,ic_index)=u90((point_index-1)*h);
  ic_matrix(2*N+point_index,ic_index)=u50((point_index-1)*h);
end
...
for point_index = 1:N
  ic_matrix(point_index,ic_index)=u40((point_index-1)*h);
  ic_matrix(N+point_index,ic_index)=u30((point_index-1)*h);
  ic_matrix(2*N+point_index,ic_index)=u50((point_index-1)*h);
end
...
for ic_index = 1:nic
  ...
  [t_arr,w,te,we,~] = ...
      ode15s(@f,tspan,w0,...
             odeset('OutputFcn',@odephas2,...
                    ...
                    'NonNegative',1:nvar,...
                    'Events',@events));
  isperiodic = length(te) > 2;
  if isperiodic
    fprintf('Решение периодическое\n');
    T = te(end) - te(end - 2);
    fprintf('T = %f\n',T);
    
    options = odeset('NonNegative',1:nvar,'RelTol',1e-5,'AbsTol',1e-8);
    outputopts = odeset('OutputFcn',@odephas2,...
                        'OutputSel',[first_oscillating_var_index,...
                                  second_oscillating_var_index]);
    figure
    calculate_multiplicators(we(end,:),T,options,outputopts);
  else
  ...
  if isperiodic
    ...
    figure
    [t_arr,wdistribution] = ...
      ode15s(@f,[0 2*T],w(end,:),...
             odeset('OutputFcn',@odephas2,...
                    'OutputSel',[first_oscillating_var_index,...
                                 second_oscillating_var_index],...
                    'NonNegative',1:nvar));
  
    analyze_solution_and_output_results(t_arr,wdistribution);
  end
  fprintf('\n');
end
...
if ~isempty(solutions)
  ...
  function u = u60( x )
    if x >= L/2 && x <= 0.9*L
      u = max(0, 0.05*sin((x-L/2)*pi/0.4/L));
    else
      u = 0;
    end
  end
  ...
  function u = u90( x )
    if x >= 0.4*L && x <= 0.8*L
      u = max(0, 0.2*sin((x-0.4*L)*pi/0.4/L));
    else
      u = 0;
    end
  end
