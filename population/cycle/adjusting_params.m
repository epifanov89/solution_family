function adjusting_params()
%ADJUSTING_PARAMS Модель взаимодействия одной популяции
...
ic_index = 1;
for point_index = 1:N
  ic_matrix(point_index,ic_index)=u30((point_index-1)*h);
  ic_matrix(N+point_index,ic_index)=u10((point_index-1)*h);
  ic_matrix(2*N+point_index,ic_index)=u20((point_index-1)*h);
end
...
for ic_index = 1:nic
  ...
  if isperiodic
    ...
    options = odeset('NonNegative',1:nvar,'RelTol',1e-6);
...
if ~isallempty
  ...
  function u = u10( x )
    u = 0.05 + 2*exp(-50*(x - L/4)^2) + exp(-50*(x - 3*L/4)^2);
  end
  % -----------------------------------------------------------------------
  
  function u = u20( x )
    u = 0.1 + 3*exp(-50*(x - L/5)^2) + 2*exp(-50*(x - 2*L/3)^2);
  end
  % -----------------------------------------------------------------------
  
  function u = u30( x )
    u = 0.05 + exp(-50*(x - L/2)^2) + 3*exp(-50*(x - 4*L/5)^2);
  end
  % -----------------------------------------------------------------------
